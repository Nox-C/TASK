...
import { Api, Bot, PnlSnapshot, TaskRun } from './api'
...
export const useDashboardStore = create<DashboardState>((set, get) => ({
  ...
  refresh: async () => {
    try {
      ...
      const [bots, pnl, taskRuns] = await Promise.all([
        Api.bots.list(),
        Api.pnl.snapshots('1d'),
        Api.tasks.recentRuns(50).catch(() => []),
      ])
      ...
      const completedRuns = taskRuns.filter(r => r.status === 'completed').length
      const failedRuns = taskRuns.filter(r => r.status === 'failed').length
      const totalFinishedRuns = completedRuns + failedRuns
      const successRate = totalFinishedRuns > 0 ? (completedRuns / totalFinishedRuns) * 100 : 0
