...
export class BacktestReportingService {
  ...
  async computeMetricsForRun(backtestRunId: string) {
    const run = await this.prisma.backtestRun.findUnique({ where: { id: backtestRunId } as any });
    ...
    const series: Array<{ ts: string; pnl: string }> = [];

    // compute equity series from fills using initial capital (if provided in params) else assume 10000
    const initialCapital = ((run.params && run.params.startBalances && Array.isArray(run.params.startBalances)) ?
      (run.params.startBalances.find((b:any)=> b.asset === 'USD')?.amount) : undefined) || 10000;

    let equity = new Decimal(initialCapital);
    let cumulative = new Decimal(0);
    for (const f of fills.sort((a:any,b:any)=> new Date(a.ts).getTime() - new Date(b.ts).getTime())) {
      ...
      cumulative = equity;
      series.push({ ts: f.ts, equity: equity.toFixed(10) });
    }
    ...
    for (const p of series) {
      const v = new Decimal(p.equity as any);
    ...
    const finalEquity = series.length ? new Decimal(series[series.length-1].equity) : new Decimal(initialCapital);
    const totalReturn = finalEquity.sub(initialCapital);
    const pctReturn = initialCapital ? totalReturn.div(initialCapital) : new Decimal(0);
    ...
    if (series.length >= 2) {
      ...
      try {
        annualized = new Decimal(1).add(pctReturn).pow(new Decimal(1).div(years)).sub(1);
      } catch (e) {
  ...
  async exportRunCsv(backtestRunId: string) {
    ...
    const rows = fills.map((f:any) => `${f.id},${f.symbol},${f.qty},${f.price},${f.ts}`).join('\n');
    return header + rows;
  }
