...
export class LedgerService {
  ...
  async recordFillAsLedger(fillId: string) {
    ...
    // fee ledger
    if (f.feeAmount && f.feeAmount !== 0) {
      await this.createEntry({ accountId, asset: f.feeAsset ?? 'USD', credit: new Decimal((f.feeAmount as any).toString()).toFixed(10), referenceType: 'fee', referenceId: fillId });
    }
